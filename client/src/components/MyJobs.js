...
export default function MyJobs({ currentCandidate, myJobs, onRemove }) {
    ...
    const jobCards = myJobs.map((job) => {

        return <div class='flex flex-col items-center'>
            <MyJobsCards key={job.id} myJobs={myJobs} job={job} onRemove={onRemove} setHideAlert={setHideAlert} />
        </div>
    })

    return (
        <div class='w-3/4 flex flex-col items-center pt-4 pb-5'>
            <h1 class="text-3xl font-bold dark:text-black pb-6">My Jobs</h1>
            <div className={hideAlert}>
                <div id="alert-2" class="fixed top-0 flex p-4 mb-4 bg-red-100 rounded-lg dark:bg-red-200" role="alert">
